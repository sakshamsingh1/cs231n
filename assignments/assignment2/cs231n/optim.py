...
def sgd_momentum(w, dw, config=None):
    ...
    v = config['momentum']*v - config['learning_rate']*dw.T
    next_w = w + v
...
def rmsprop(w, dw, config=None):
    ...
    config['cache'] = config['decay_rate']*config['cache'] + (1-config['decay_rate'])*(dw*dw).T
    next_w = w - (config['learning_rate']*dw.T/(np.sqrt(config['cache'])+config['epsilon']))
